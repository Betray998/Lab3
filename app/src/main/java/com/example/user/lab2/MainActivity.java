    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        play.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                else if(mora_gmaer == -1)
                    status.setText("請選擇出拳種類");
                else{
                    name.setText(gamer.getText());
                    myMora.setText(MoraString[mora_gmaer]);
                    int mora_computer = (int) (Math.random() * 3);
                    computerMora.setText(MoraString[mora_computer]);
                    if((mora_gmaer ==0 && mora_computer == 1) || (mora_gmaer == 1 && mora_computer == 2) || (mora_gmaer == 2 && mora_computer == 0))
                    {
                        winner.setText("電腦");
                        status.setText("可惜,電腦獲勝了");
                    } else if(mora_computer == mora_gmaer){
                        winner.setText("平局");
                        status.setText("平局! 再玩一場看看"); }
                        else {
